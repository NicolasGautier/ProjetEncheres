...
<jsp:include page="navbar.jsp" />
					...
					<h1 class="mb5 text-center">Liste des enchères</h1>	
  	...
  	<div class="row align-items-center">
  		<div class="col-md-3">
 			...
 			<label for="filtre">Filtres :</label>
  			...
  			<p><label for="categorieSelect">Catégorie :</label>
  		 	<select
 		...
 		<div class="col-md-3 ml-md-auto">
  			<button type="submit" value="Rechercher" class="btn-secondary btn-lg btn-block">Rechercher</button>
  		</div>
 	</div> 
			...
			<c:forEach items="${accModel.lstArticleVendu}" var="articleVendu">
					<div class="card" style=" max-width: 25rem;">				
						...
						src="<%=request.getContextPath()%>/image/image_informatique.png">
							...
							<div class= "col-6">
									<div class="card-body">
									<h5 class="card-title"><a href="<c:if test="${articleVendu.etatVente != accModel.etatsVenteEncheresTerminees}">DetailVenteServlet</c:if>
									<c:if test="${articleVendu.etatVente == accModel.etatsVenteEncheresTerminees}">VenteRemporteServlet</c:if>?id=${articleVendu.noArticle}">${articleVendu.nomArticle}</a></h5>
									<p class="card-text">${articleVendu.prixVente} points</p>
									<p class="card-text">${articleVendu.dateFinEncheres}</p>
									<p class="card-text"><a
										href="ProfilServlet?id=${articleVendu.utilisateurVend.noUtilisateur}">${articleVendu.utilisateurVend.pseudo}</a></p>
									</div>
							</div>
						</div>
					</div>
